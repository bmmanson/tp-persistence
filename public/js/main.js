$(function () {
    ...
    populateDropdown('hotels', $('#hotel-choices'));
    populateDropdown('restaurants', $('#restaurant-choices'));
    populateDropdown('activities', $('#activity-choices'));
    ...
    function createDayButton(number) {
        $.post('/api/days/' + number, {number: number})
    ...
    function reRenderDayButtonsAJAX() {
...
reRenderDayButtonsAJAX();
    ...
    // AJAX functions ----
    function populateDropdown(type, $selectElement) {
        $.ajax({
            method: "GET",
            url: "/api/" + type
            })
        .then(function(items){
            fillInOptions(items, $selectElement);
           })
    }
